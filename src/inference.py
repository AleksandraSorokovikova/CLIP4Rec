...
class Inference:
    ...
    def get_embeddings(self, film_descriptions_encoded):
        ...
        for i in tqdm(range(0, len(all_film_indices), batch_size)):
            ...
            for j in range(i, min(i + batch_size, len(all_film_indices))):
                film_id = torch.tensor([all_film_indices[j]], dtype=torch.long).to(device)
                film_id = film_id.unsqueeze(0)
                film_ids.append(film_id)

                encoded_description = film_descriptions_encoded[all_film_indices[j]]
                description = encoded_description['input_ids'].unsqueeze(0).to(device)
                attention_mask = encoded_description['attention_mask'].unsqueeze(0).to(device)
    ...
    def build_annoy_model(self, search_type='euclidian'):
        print('Counting embeddings...')
        self.film_embeddings, self.text_embeddings = self.get_embeddings(film_descriptions_encoded)
        print('Building Annoy model...')
        self.annoy_model = AnnoySearchEngine(
            self.film_embeddings, 
